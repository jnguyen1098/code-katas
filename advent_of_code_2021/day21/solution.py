    ...
    for roll, freq in ROLL_DISTRIBUTION.items():
        if current_player == ONE:
            ...
            if new_sco >= 21:
                ...
                incr1, incr2 = get_universes(encode(new_pos, new_sco, position_of_player[TWO], s2, TWO))
                p1_wins += incr1 * freq
                p2_wins += incr2 * freq
            ...
            if new_sco >= 21:
                ...
                incr1, incr2 = get_universes(encode(position_of_player[ONE], s1, new_pos, new_sco, ONE))
                p1_wins += incr1 * freq
                p2_wins += incr2 * freq
